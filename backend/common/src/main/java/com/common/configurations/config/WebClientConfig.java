...
public class WebClientConfig {
        ...
        public WebClient webClientPoint(WebClient.Builder builder) {
                return builder
                                ...
                                .filter((request, next) -> ReactiveSecurityContextHolder.getContext()
                                                .map(securityContext -> {
                                                        Jwt token = (Jwt) securityContext.getAuthentication()
                                                                        .getCredentials();
                                                ...
                                                .flatMap(next::exchange) // Continua la chiamata dopo aver aggiunto il
                                                                         // token
                                )
                                .build();
        }
        ...
        @Bean
        public WebClient webClientYoutube(@Value("${youtube.base-url}") String baseUrl) {
